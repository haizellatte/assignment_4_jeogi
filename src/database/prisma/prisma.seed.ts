...
async function seed() {
  ...
  //! map의 결과로 나온 프로미스 객체를 "병렬적으로 기다려주기 위해" Promise.all에 각 프로미스 객체를 전달한다.
  //! migrate 할 때마다 seed가 심어지는 것을 방지하기 위해 create 대신 --> upsert로 !
  const result = await Promise.all(
    regions.map((region) =>
      prismaClient.regions.upsert({
...
//* 방법2
// ╰─ node ./src/database/prisma/prisma.seed.ts
